...
Sub2API 平台新增「套餐订阅」功能：将多个 AI 模型（通过现有 Group 体系）捆绑成套餐售卖，每个 Group 可独立配置日/周/月额度。支持多种套餐层级（基础/旗舰/企业），用户购买后获得套餐内所有模型的访问权。
...
|---|---|---|
| `name` | string | 套餐名称（如"基础款"、"旗舰款"） |
| `description` | string | 套餐描述 |
| `tier` | string | 套餐层级: `basic` / `flagship` / `enterprise` |
| `price` | float64 | 售价 |
