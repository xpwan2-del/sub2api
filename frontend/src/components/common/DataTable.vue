    ...
    <table class="w-full min-w-max divide-y divide-gray-200 dark:divide-dark-700">
      ...
      <tbody class="table-body divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
        ...
        <template v-else>
          ...
          <!-- Row expansion (only when not virtualized) -->
          <template v-if="!shouldVirtualize && expandedRowKey !== null">
            <template v-for="(row, index) in sortedData" :key="'expand-' + resolveRowKey(row, index)">
              <tr
                v-if="expandedRowKey === resolveRowKey(row, index)"
                class="expanded-row"
              >
                <td :colspan="tableColumnCount" class="bg-gray-50 dark:bg-dark-800/50">
                  <slot name="row-expansion" :row="row" />
                </td>
              </tr>
            </template>
          </template>
...
interface Props {
  ...
  overscan?: number
  /** 启用虚拟滚动（默认 true）。数据量小的分页页可设为 false 以支持行内展开。 */
  virtual?: boolean
  /** 当前展开行的 key；与 row-expansion 插槽配合，仅在 virtual=false 时生效 */
...
const shouldVirtualize = computed(() =>
  props.virtual && isDesktopViewport.value && (sortedData.value?.length ?? 0) > (props.virtualizeThreshold ?? 100)
)
