<template>
  ...
  >
        ...
        <div class="sidebar-section">
          <template v-for="item in adminNavItems" :key="item.path">
            ...
            <template v-if="item.children?.length">
              <button
                type="button"
                class="w-full mb-1 sidebar-link"
                :class="{
              ...
              <!-- Children -->
              <div v-if="!sidebarCollapsed && isGroupExpanded(item)" class="pl-2 mb-1 ml-4 border-l border-gray-200 dark:border-dark-600">
                <router-link
            ...
            >
              <span v-if="item.iconSvg" class="flex-shrink-0 w-5 h-5 sidebar-svg-icon" v-html="sanitizeSvg(item.iconSvg)"></span>
              <component v-else :is="item.icon" class="flex-shrink-0 w-5 h-5" />
    ...
    <!-- Bottom Section -->
    <div class="p-3 mt-auto border-t border-gray-100 dark:border-dark-800">
      <!-- Theme Toggle -->
      <button
        @click="toggleTheme"
        class="w-full mb-2 sidebar-link"
        :class="{ 'sidebar-link-collapsed': sidebarCollapsed }"
        :title="sidebarCollapsed ? (isDark ? t('nav.lightMode') : t('nav.darkMode')) : undefined"
      >
        <SunIcon v-if="isDark" class="flex-shrink-0 w-5 h-5 text-amber-500" />
      ...
      >
        <ChevronDoubleLeftIcon v-if="!sidebarCollapsed" class="flex-shrink-0 w-5 h-5" />
        <ChevronDoubleRightIcon v-else class="flex-shrink-0 w-5 h-5" />
...
// 2nd-dev: flagSubscription unused here — subscription nav entries are commented out in
// favour of the bundle system; the subscription_enabled gate still guards the header
// subscription badge (AppHeader.vue) and the purchase tabs (PaymentView.vue).
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    // 2nd-dev: subscription entries replaced by the bundle system; purchase label still follows
    // the upstream site-billing-mode resolver (defaults to buySubscription on bundle sites).
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/model-status', label: t('nav.modelStatus'), icon: SignalIcon, featureFlag: flagOpsMonitoring },
    ...
    },
    {
      ...
      label: t('nav.upstreamSync', 'Upstream Sync'),
      icon: CloudIcon,
      hideInSimpleMode: true,
      expandOnly: true,
      children: [
        { path: '/admin/upstream-sources', label: t('nav.upstreamSources', 'Upstream Sources'), icon: CloudIcon },
        { path: '/admin/price-change-requests', label: t('nav.priceChangeRequests', 'Price Change Requests'), icon: ClipboardIcon },
      ],
    },
    // 2nd-dev: admin subscription management replaced by the bundle system (see note above).
    ...
    { path: '/admin/catalog', label: t('nav.adminCatalog'), icon: GridIcon },
