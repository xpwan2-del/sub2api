      ...
      <template v-else>
        ...
        <template v-else>
          <!-- Neither top-up nor subscriptions available (balance recharge disabled via API while subscriptions are off).
               2nd-dev: skipped in bundle mode — the inline bundle purchase card below takes over. -->
          <div v-if="tabs.length === 0 && !isBundleMode" class="card py-16 text-center">
            <p class="text-gray-500 dark:text-gray-400">{{ t('payment.billingUnavailable') }}</p>
          </div>
          <!-- Top-up Tab (hidden in bundle mode) -->
          <template v-else-if="activeTab === 'recharge' && !isBundleMode">
