            ...
            <div v-else class="space-y-3">
              <div v-for="method in stats.payment_methods" :key="method.type" class="flex items-center justify-between">
                ...
                <div class="space-y-1 text-right">
                  <div v-for="[currency, amount] in sortedAmounts(method.amount)" :key="currency" class="flex items-center justify-end gap-2">
                    <span class="text-sm font-medium text-gray-900 dark:text-white">{{ formatMoney(currency, amount) }}</span>
                    <span v-if="method.balance_amount?.[currency]" class="text-xs text-gray-500 dark:text-gray-400">/{{ formatMoney(currency, method.balance_amount[currency]) }}</span>
                  </div>
