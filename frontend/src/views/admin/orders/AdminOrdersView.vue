        ...
        <template #actions="{ row }">
          <div class="flex items-center gap-1">
            ...
            <template v-if="row.status === 'REFUND_REQUESTED'">
              <span v-if="row.refund_amount" class="rounded-full bg-purple-100 px-1.5 py-0.5 text-xs font-medium text-purple-700 dark:bg-purple-900/30 dark:text-purple-300">{{ row.order_type === 'balance' ? '$' : '¥' }}{{ (row.refund_amount ?? 0).toFixed(2) }}</span>
              <button :disabled="row.payment_type === 'balance'" @click="row.payment_type !== 'balance' && openRefundDialog(row)" :class="row.payment_type === 'balance' ? 'cursor-not-allowed text-gray-400 dark:text-gray-600' : 'text-purple-600 hover:bg-purple-50 dark:text-purple-400 dark:hover:bg-purple-900/20'" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium">
                <Icon name="check" size="sm" />
                {{ t('payment.admin.approveRefund') }}
              </button>
            </template>
            <button v-else-if="row.status === 'REFUND_FAILED'" :disabled="row.payment_type === 'balance'" @click="row.payment_type !== 'balance' && openRefundDialog(row)" :class="row.payment_type === 'balance' ? 'cursor-not-allowed text-gray-400 dark:text-gray-600' : 'text-purple-600 hover:bg-purple-50 dark:text-purple-400 dark:hover:bg-purple-900/20'" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium">
              <Icon name="refresh" size="sm" />
              {{ t('payment.admin.retryRefund') }}
            </button>
            <button v-else-if="row.status === 'COMPLETED' || row.status === 'PARTIALLY_REFUNDED'" :disabled="row.payment_type === 'balance'" @click="row.payment_type !== 'balance' && openRefundDialog(row)" :class="row.payment_type === 'balance' ? 'cursor-not-allowed text-gray-400 dark:text-gray-600' : 'text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20'" class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium">
    ...
    <BaseDialog :show="showDetailDialog" :title="t('payment.admin.orderDetail')" width="wide" @close="showDetailDialog = false">
      <div v-if="selectedOrder" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.amount') }}</p><p class="text-sm font-medium text-gray-900 dark:text-white">{{ selectedOrder.order_type === 'balance' ? '$' : '¥' }}{{ (selectedOrder.amount ?? 0).toFixed(2) }}</p></div>
          <div><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.orders.payAmount') }}</p><p class="text-sm font-medium text-gray-900 dark:text-white">¥{{ (selectedOrder.pay_amount ?? 0).toFixed(2) }}</p></div>
          ...
          <div v-if="selectedOrder.payment_type === 'balance'"><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.balanceDeductAmount') }}</p><p class="text-sm font-medium text-blue-600 dark:text-blue-400">${{ (selectedOrder.balance_deduct_amount ?? 0).toFixed(2) }}</p></div>
          ...
          <div v-if="selectedOrder.refund_amount"><p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.admin.refundAmount') }}</p><p class="text-sm font-medium text-red-600 dark:text-red-400">{{ selectedOrder.order_type === 'balance' ? '$' : '¥' }}{{ (selectedOrder.refund_amount ?? 0).toFixed(2) }}</p></div>
