...
// TestGatewayHandlerSubmitUsageRecordTaskForMedia_ImageResultUsesMandatoryFallback
// 真实场景：Gemini 图片生成（Imagen）经兼容路径产出 ForwardResult.ImageCount>0
// （gemini_messages_compat_service.go 构造 ForwardResult 时填充 ImageCount），其计费任务
// 不可被 worker 池丢弃，否则 image count 永不增长、图片次数限额失效。H3-ext 让通用网关
// 的媒体计费走 mandatory。
func TestGatewayHandlerSubmitUsageRecordTaskForMedia_ImageResultUsesMandatoryFallback(t *testing.T) {
	pool := service.NewUsageRecordWorkerPoolWithOptions(service.UsageRecordWorkerPoolOptions{
		WorkerCount:           1,
		QueueSize:             1,
		TaskTimeout:           time.Second,
	...
	var called atomic.Bool
	h.submitUsageRecordTaskForMedia(context.Background(), &service.ForwardResult{ImageCount: 1}, func(ctx context.Context) {
		called.Store(true)
	})
	close(release)
...
// TestGatewayHandlerSubmitUsageRecordTaskForMedia_VideoResultUsesMandatoryFallback
// 验证 VideoCount 分支的前瞻逻辑：通用网关当前不处理视频（ForwardResult.VideoCount 恒为 0，
// 视频走 OpenAI /v1/videos 由 H3 覆盖），但保留 VideoCount 判定以备将来通用网关支持视频
// 产出时自动生效。
func TestGatewayHandlerSubmitUsageRecordTaskForMedia_VideoResultUsesMandatoryFallback(t *testing.T) {
	pool := service.NewUsageRecordWorkerPoolWithOptions(service.UsageRecordWorkerPoolOptions{
		WorkerCount:           1,
		QueueSize:             1,
		TaskTimeout:           time.Second,
	...
	var called atomic.Bool
	h.submitUsageRecordTaskForMedia(context.Background(), &service.ForwardResult{VideoCount: 1}, func(ctx context.Context) {
		called.Store(true)
	})
	close(release)
...
// TestGatewayHandlerSubmitUsageRecordTaskForMedia_NonMediaUsesDroppablePath
// 守护不误伤：无媒体产出（nil result 或 ImageCount=VideoCount=0）的文本请求仍走可丢弃的
// 普通路径，任务由 worker 池正常执行，不会被强制同步。
func TestGatewayHandlerSubmitUsageRecordTaskForMedia_NonMediaUsesDroppablePath(t *testing.T) {
	pool := newUsageRecordTestPool(t)
	h := &GatewayHandler{usageRecordWorkerPool: pool}

	done := make(chan struct{})
	// nil result 与纯文本 result 都应走普通路径并被池执行。
	h.submitUsageRecordTaskForMedia(context.Background(), nil, func(ctx context.Context) {
		close(done)
	})
	select {
	case <-done:
	case <-time.After(time.Second):
		t.Fatal("non-media task should be enqueued and executed via droppable path")
	}
}
