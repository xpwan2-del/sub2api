...
const apiKeyAuthSnapshotVersion = 25 // v25: merge upstream v24 (group model_allowlist, renamed from models_list_config) on top of 2nd-dev v24 (upstream v23 codex manifest + bundle/video4k/free_openai_fast) — force refresh
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		BundleSubscriptionID: apiKey.BundleSubscriptionID,
		Name:                 apiKey.Name,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		BundleSubscriptionID: snapshot.BundleSubscriptionID,
		Key:                  key,
		Name:                 snapshot.Name,
