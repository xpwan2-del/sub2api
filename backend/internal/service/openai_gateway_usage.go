...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	// 独立图片倍率：token 计价但产出图片的请求（gpt-image-2 等）按图片倍率扣费
	// （41bae2fcd 引入；上次合并上游 peak-aware 重构时丢失，在此恢复）。
...
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCost(
	...
	if s.isOpenAIVideoUsage(ctx, result, billingModels, apiKey) || isGrokVideoUsageResult(result, billingModels) {
...
func (s *OpenAIGatewayService) calculateOpenAIVideoCost(
	...
	if resolved != nil {
		...
		if resolved.Mode == BillingModePerRequest || resolved.Mode == BillingModeImage || resolved.Mode == BillingModeVideo {
			...
			// video 模式 tier 单价为每秒价，用量 = 段数 × 时长秒数。
			gid := apiKey.Group.ID
