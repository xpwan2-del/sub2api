...
func (s *PaymentService) ExecuteBundleFulfillment(ctx context.Context, oid int64) error {
	...
	// reload 订单获取 Recharging 状态的 UpdatedAt，作为 fulfillment lease version
	// 桥接分支 DB 锁路径（StatusIn+SetStatus）到 main lease 机制（UpdatedAtEQ 版本校验）
	o, err = s.entClient.PaymentOrder.Get(ctx, oid)
	if err != nil {
		return fmt.Errorf("reload after lock: %w", err)
	}
	if err := s.doBundle(ctx, o); err != nil {
		s.markFailed(ctx, oid, &paymentFulfillmentLease{version: o.UpdatedAt}, err)
		return err
	}
	return nil
}
...
func (s *PaymentService) refundUpgradeToBalance(ctx context.Context, o *dbent.PaymentOrder) error {
	...
	action, err := resolveRedeemAction(existing, lookupErr)
	if err != nil {
		return fmt.Errorf("lookup upgrade refund redeem code: %w", err)
	}
	switch action {
	case redeemActionSkipCompleted:
