...
type BundleUsageRepository interface {
	...
	// 用 now 初始化一条空 usage（usage 值全 0、window_start 对齐到 now 所在自然日 0 点）并返回；已存在则原样返回（不修改累计值）。
	...
	// IncrementUsage 原子累加 costUSD/imageCount/videoCount 到日/周/月三个滚动窗口（自然日 0 点对齐）。
	// 窗口是否过期由 repo 在事务内基于 DB 真实 window_start 判断（FOR UPDATE 锁行），
	// 过期则置为本次值并把 window_start 推进到 now 所在自然日 0 点，否则在原值上累加。now 为本次累加基准时刻。
