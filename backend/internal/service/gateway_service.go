...
// BundleSubID 标记 bundle 订阅归属（防跨订阅 IDOR）；OwnerUserID 标记标准 Key 用户归属。
...
// GetBundleAvailableModels 返回某套餐订阅下所有可用模型（聚合各 group quota 的模型，按 model pattern 过滤）。
func (s *GatewayService) GetBundleAvailableModels(ctx context.Context, bundleSubID int64) []string {
	if s.bundleUsageService == nil {
		return nil
	}
