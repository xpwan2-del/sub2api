...
// shouldAccumulateBundleUsage 判断本次计费是否应累加到套餐子系统（套餐订阅 + 有成本或媒体产出）。
// 按次计费与成本解耦：ActualCost>0 或媒体产出>0 即累加，避免 0 定价媒体不计数导致次数限额失效。
...
func postUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps) {
	...
	if p.IsSubscriptionBill {
		...
		// Bundle usage accumulation: 套餐订阅计费（按 ActualCost + 图片/视频产出计数累加到套餐子系统）
...
func finalizePostUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	if p.IsSubscriptionBill {
		...
		// Bundle usage accumulation: 套餐订阅用量累加到套餐子系统（生产计费路径，
		// applyUsageBilling → repo.Apply 成功后在此收尾）。按 ActualCost + 图片/视频产出计数累加。
		if deps.bundleUsageService != nil && p.shouldAccumulateBundleUsage() {
