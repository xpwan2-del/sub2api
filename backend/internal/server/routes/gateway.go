...
func RegisterGatewayRoutes(
	...
	// 之前，避免逐条路由手工维护链导致漏挂。bundle key 的目标分组由
	// bundleRouteResolver 注入，必须先于 groupModelAllowlist 执行，使白名单
	// 基于注入后的分组判定。
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), bundleRouteResolver.BundleResolver(), groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)
	}
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), bundleRouteResolver.BundleResolver(), groupModelAllowlist, compositeTarget, requireGroupAnthropic)
	{
