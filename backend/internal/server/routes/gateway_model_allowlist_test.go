...
func newGatewayRoutesTestRouterWithGroup(group *service.Group) *gin.Engine {
	...
	RegisterGatewayRoutes(
		...
		nil, // bundleRouteResolver（二开新增参数，此测试不涉套餐路由）
		nil,
		nil,
		nil,
		nil,
...
func TestGatewayRoutesGroupModelAllowlistMountedOnEveryGatewayRoute(t *testing.T) {
	...
	// rootRoute helper：apiKeyAuth 之后、compositeTarget 之前。二开链在 auth 与 allowlist
	// 之间插入 bundleRouteResolver（套餐 key 的目标分组注入必须先于白名单判定）。
	rootHelper := regexp.MustCompile(regexp.QuoteMeta(`r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), bundleRouteResolver.BundleResolver(), groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)`))
	require.Regexp(t, rootHelper, source,
		"root alias helper must place bundleRouteResolver and the allowlist between apiKeyAuth and compositeTarget")
	...
	// codexDirect 链是一条 Use 调用，直接断言顺序（含二开 bundleRouteResolver 注入）。
	codexDirect := regexp.MustCompile(regexp.QuoteMeta(`codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), bundleRouteResolver.BundleResolver(), groupModelAllowlist, compositeTarget, requireGroupAnthropic)`))
